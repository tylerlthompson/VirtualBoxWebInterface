...
import VirtualMachineDriver, socket, os, Logger
    ...
    if not session.get('logged_in'):
        ...
        if curVmInfo == None:
            try:
                ...
                generateMainPageNew() 
                return render_template('mainpagenew.html', hostname=hostname, version=version, name="", vmstate="", os="", mem="", cpu="", harddrive="", harddrivesize="", cddrive="", networkadapter="", mac="")
...
def do_admin_login():
    ...
    if password == 'password' and username == 'admin':
        ...
        log.log(username + " logged in successfully.")
    else:
        log.log(username + " did not login successfully.")
...
def action():
    ...
    if action == 'Start':
        ...
        if vmd.getVmState(curVmInfo) == "running":
            return render_template('error.html', message='The Virtual Machine must be turned off before changes can be saved.')
        else:
            oldName = curVmInfo
...
def generateMainPageNew():
    ...
    peice4 = generateOsTypes()
    
    target = open('./static/peice5.html', 'r')
    peice5 = target.read()
    target.close()

    peice6 = generateMemoryList()
    
    target = open('./static/peice7.html', 'r')
    peice7 = target.read()
    target.close()
    
    peice8 = generateCpuList()
    
    target = open('./static/peice9.html', 'r')
    peice9 = target.read()
    target.close()
    
    peice10 = generateNetworkAdapterList()
    ...
    s = peice1 + peice2 + peice3 + peice4 + peice5 + peice6 + peice7 + peice8 + peice9 + peice10 + peice11 + peice12 + peice13 + peice14 + peice15 + peice16 + peice17 + peice18 + peice19
    target = open('./templates/mainpagenew.html', 'w')
    target.write(s)
    target.close()
...
def generateVmList():
    ...
    for i in range(len(vms)):
        s = s + "<tr><td class='buttonCell'><form action='/' method='POST'><input class='VmButton' name='text' type='submit' value='" + vms[i].name + "' /></form></td></tr>"
    s = s + "</table></div>"
    return s
